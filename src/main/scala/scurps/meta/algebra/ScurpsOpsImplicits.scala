...
trait ScurpsOpsImplicits {
  final implicit class RichAlgebraic[A[+_],T](v:A[T]) {
    ...
    @inline def orElse(defaultValue:A[T])(implicit ops:ScurpsOps[A]):A[T] = ??? // TODO
  }

  final implicit class RichAlgebraicContext[A[+_]](v:A[GameContext]) {
    @inline def get[T](contextKey:ContextKey[T])(implicit ops:ScurpsOps[A]):A[T] = ??? // TODO
  }

  final implicit class RichAlgebraicPMap[A[+_],K[_]](v:A[PMap[K]]) {
    @inline def get[T](key:K[T])(implicit ops:ScurpsOps[A]):A[T] = ??? // TODO
    @inline def get[T,V](valueToWrap:A[V])(implicit wrapper:KeyWrapper[K[T],V]):A[T] = ??? // TODO
  }
