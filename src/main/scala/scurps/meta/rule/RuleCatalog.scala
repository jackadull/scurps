...
trait RuleCatalog {
  def +[P[_[_]],R](kv:(RuleKey[P,R],Rule[P,R])):RuleCatalog
  def ++(kvs:Seq[(RuleKey[P,R],Rule[P,R]) forSome {type P[_[_]];type R}]):RuleCatalog
  def get[P[_[_]],R](key:RuleKey[P,R]):Option[Rule[P,R]]
  def isEmpty:Boolean
}
object RuleCatalog {
  val empty:RuleCatalog = MapRuleCatalog(Map.empty)
  def apply(kvs:(RuleKey[P,R],Rule[P,R]) forSome {type P[_[_]];type R}*):RuleCatalog = empty.++(kvs)

  private final case class MapRuleCatalog(map:Map[Any,Rule[Nothing,Any]]) extends RuleCatalog {
    override def +[P[_[_]],R](kv:(RuleKey[P,R],Rule[P,R])):MapRuleCatalog = map + kv match {
