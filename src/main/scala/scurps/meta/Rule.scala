...
object Rule {
  ...
  trait Rule0[+R] extends Rule {
    ...
    def forAny[T1]:Rule1[T1,R] = ForAny(this)
  }
  ...
  final case class ForAny[-T1,+R](inner:Rule0[R]) extends Rule1[T1,R] {
    override def apply(v1:T1)(implicit context:GameContext):Derivation[R] = Derivation.ForAny(inner())
  }
}
