...
object RuleKey {
  trait RuleKey0[+R] extends RuleKey[Rule0[R]] {
    def derivation()(implicit context:GameContext):Derivation[R] = context.ruleCatalog.get(this) match {
      case Some(rule) => rule()
      case None => MissingRule(this)
    }
  }
  trait RuleKey1[-T1,+R] extends RuleKey[Rule1[T1,R]] {
    def derivation(v1:T1)(implicit context:GameContext):Derivation[R] = context.ruleCatalog.get(this) match {
